...
- **Packet grouping** — groups every reception by message, so you can see at a glance which repeaters forwarded the same packet and compare their signal side by side. Packets are decoded with a lean patched fork of `@michaelhart/meshcore-decoder` ([github.com/kybl/meshcore-decoder](https://github.com/kybl/meshcore-decoder)) — type, path, last-hop repeater IDs, RSSI, SNR, payload fields — to drive the grouping and labelling. The fork adds public-channel message decryption, corrected Path decoding, and per-hop SNR on Trace packets
- **Seen repeaters table** — per-repeater statistics (RX count, max/last RSSI, max/last SNR, last seen); sortable columns
- **SNR & RSSI history charts** — scrolling time-series per repeater; the RSSI chart shades the per-packet noise floor estimate (RSSI − SNR) and draws the radio's own measured noise floor (polled every ~10 s) as a dashed line that also covers quiet gaps with no packets; click a chart dot to highlight one repeater across all views and open that packet's row in the table; hover or tap a point for a tooltip with its exact SNR/RSSI, millisecond time and packet type (and the reception count for a clustered point); **zoom and pan the time axis** (mouse wheel, drag across a region, or pinch — both charts share one time window; double-click or **Reset zoom** to restore), and while measuring a zoomed view kept at the right edge keeps following new packets
...
- **Location capture** — every packet is geotagged for the 3D map. A **Packet position from** selector chooses the source: *This phone / browser* (the default) uses this device's own GPS — in the Android app this starts automatically once you grant the location permission while connecting, and keeps running in the background; *MeshCore device* instead stamps packets with the position the connected radio reports, needing no phone location permission — handy when the radio moves separately from you (on a car roof, a dog's collar…). One-off GPS outliers are filtered out so your position marker doesn't jump
...
- **Sound** — optional per-packet audio (off / **disconnect only** / short / medium / long, the last three of increasing duration). A packet with a **new** hash opens with a short wooden knock; every reception — including repeats of the same packet arriving via other repeaters — then adds a bell note whose pitch scales with SNR (0 dB = base, ±10 dB = ±1 octave), so one packet heard via five repeaters plays one knock and five pitched notes. *disconnect only* stays silent per-packet but still sounds the disconnect alarm, and any non-off setting sounds it too; when a repeater filter is active, only the filtered repeater(s) trigger sound
...
- **Device identity & radio preset** — the connection status box shows the connected node's name and, matched against a built-in table of the standard MeshCore radio presets (frequency, bandwidth, spreading factor, coding rate), which preset the radio is running; the official online list is consulted only when the radio reports settings the built-in table doesn't recognise (at most once a day), so most installs never make that request at all
- **Dated timestamps** — packet times show just the clock when they fall on today's date and add the date when they don't, so an overnight or imported capture stays unambiguous (the switch is midnight-safe)
...
- [MeshCore Decoder](https://github.com/michaelhart/meshcore-decoder) — TypeScript library for packet decoding ([patched fork](https://github.com/kybl/meshcore-decoder) used here)

...
- **Device identity & radio preset** — the connection status box shows the connected node's name and, matched against a built-in table of the standard MeshCore radio presets (frequency, bandwidth, spreading factor, coding rate), which preset the radio is running; the preset table is refreshed at runtime from the official online list with the built-in copy as fallback
...
- [MeshCore Decoder](https://github.com/michaelhart/meshcore-decoder) — TypeScript library for packet decoding ([patched fork](https://github.com/kybl/meshcore-decoder/tree/patched) used here)
